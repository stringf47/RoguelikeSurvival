const express = require('express');
const bcrypt  = require('bcrypt');
const { db }  = require('../db');

const router = express.Router();

function requireAdmin(req, res, next) {
  if (!req.session.user || !req.session.user.isAdmin)
    return res.status(403).json({ error: 'Forbidden' });
  next();
}

router.use(requireAdmin);

/* ── USERS ── */

router.get('/users', async (req, res) => {
  const rows = await db.all(
    `SELECT u.id, u.username, u.is_guest, u.is_admin, u.created_at,
            COUNT(s.id)::int AS run_count
     FROM users u
     LEFT JOIN scores s ON s.user_id = u.id
     GROUP BY u.id
     ORDER BY u.created_at DESC`
  );
  res.json(rows);
});

router.delete('/users/:id', async (req, res) => {
  const id = Number(req.params.id);
  if (id === req.session.user.id)
    return res.status(400).json({ error: 'Cannot delete your own account' });
  const user = await db.get('SELECT id FROM users WHERE id = ?', [id]);
  if (!user) return res.status(404).json({ error: 'User not found' });
  await db.run('DELETE FROM users WHERE id = ?', [id]);
  res.json({ ok: true });
});

router.post('/users/:id/reset-password', async (req, res) => {
  const id = Number(req.params.id);
  const { newPassword } = req.body;
  if (!newPassword || newPassword.length < 6)
    return res.status(400).json({ error: 'Password must be at least 6 characters' });
  const user = await db.get('SELECT id, is_guest FROM users WHERE id = ?', [id]);
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (user.is_guest) return res.status(400).json({ error: 'Guest accounts have no password' });
  const hash = await bcrypt.hash(newPassword, 10);
  await db.run('UPDATE users SET password = ? WHERE id = ?', [hash, id]);
  res.json({ ok: true });
});

router.post('/users/:id/promote', async (req, res) => {
  const id = Number(req.params.id);
  if (id === req.session.user.id)
    return res.status(400).json({ error: 'Cannot change your own admin status' });
  const user = await db.get('SELECT id, is_admin FROM users WHERE id = ?', [id]);
  if (!user) return res.status(404).json({ error: 'User not found' });
  const newVal = user.is_admin ? 0 : 1;
  await db.run('UPDATE users SET is_admin = ? WHERE id = ?', [newVal, id]);
  res.json({ ok: true, is_admin: newVal });
});

/* ── SCORES ── */

router.get('/scores', async (req, res) => {
  const rows = await db.all(
    `SELECT s.id, s.username, s.time, s.kills, s.level, s.damage, s.dps, s.won, s.created_at,
            u.is_guest, u.is_admin
     FROM scores s
     JOIN users u ON u.id = s.user_id
     ORDER BY s.created_at DESC LIMIT 500`
  );
  res.json(rows);
});

router.delete('/scores/:id', async (req, res) => {
  const id = Number(req.params.id);
  const score = await db.get('SELECT id FROM scores WHERE id = ?', [id]);
  if (!score) return res.status(404).json({ error: 'Score not found' });
  await db.run('DELETE FROM scores WHERE id = ?', [id]);
  res.json({ ok: true });
});

module.exports = router;
